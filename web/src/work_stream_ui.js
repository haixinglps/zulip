...
import render_work_stream_overlay from "../templates/stream_settings/work_stream_overlay.hbs";
import render_work_streams_list from "../templates/stream_settings/work_streams_list.hbs";
import render_work_streams_detail from "../templates/stream_settings/work_streams_detail.hbs";
...
import * as work_stream_edit from "./work_stream_edit";
...
export function set_right_panel_title(sub) {
    ...
    $("#work_subscription_overlay .stream-info-title").html(
...
export const show_subs_pane = {
    nothing_selected() {
        ...
        $("#work_subscription_overlay .stream-info-title").text($t({defaultMessage: "Stream settings"}));
    },
    settings(sub) {
        $(".settings, #work_stream-creation").hide();
        $(".settings").show();
    ...
    create_stream() {
        ...
        $("#work_subscription_overlay .stream-info-title").text($t({defaultMessage: "Create stream"}));
    },
};
...
export function set_muted(sub, is_muted, status_element) {
    ...
    work_stream_edit.set_stream_property(sub, "is_muted", sub.is_muted, status_element);
}

export function toggle_pin_to_top_stream(sub) {
    work_stream_edit.set_stream_property(sub, "pin_to_top", !sub.pin_to_top);
}
...
export function update_stream_name(sub, new_name) {
    ...
    work_stream_edit.update_stream_name(sub, new_name);
...
export function update_stream_description(sub, description, rendered_description) {
    ...
    work_stream_edit.update_stream_description(sub);
...
export function set_color(stream_id, color) {
    ...
    work_stream_edit.set_stream_property(sub, "color", color);
}
...
export function add_sub_to_table(sub) {
    ...
    scroll_util
        .get_content_element($("#work_streams_overlay_container .settings"))
...
export function remove_stream(stream_id) {
    ...
    if (hash_util.is_editing_stream(stream_id)) {
        work_stream_edit.open_edit_panel_empty();
    }
}
...
export function render_left_panel_superset() {
    ...
    scroll_util.get_content_element($("#work_streams_overlay_container .streams-list")).html(html);
}
...
export function update_empty_left_panel_message() {
    ...
    if (is_subscribed_stream_tab_active()) {
        ...
        has_streams =
            ...
            $("#work_streams_overlay_container .stream-row:not(.notdisplayed)").length;
    } else {
...
export function redraw_left_panel(left_panel_params = get_left_panel_params()) {
    ...
    for (const row of $("#work_streams_overlay_container .stream-row")) {
    ...
    scroll_util.reset_scrollbar($("#work_subscription_overlay .streams-list"));

    const all_stream_ids = [...buckets.name, ...buckets.desc, ...buckets.other];

    for (const stream_id of all_stream_ids) {
        scroll_util
            .get_content_element($("#work_streams_overlay_container .streams-list"))
...
export function get_left_panel_params() {
    const $search_box = $("#work_stream_filter input[type='text']");
...
let myStreamPage = 1;
let myStreamsTotal = 0;
let allStreamPage = 1;
let allStreamsTotal = 0;
const pageSize = 10;
const emptyList = [{
    pid: '',
    img: '',
...
export function switch_stream_tab(tab_name) {
    ...
    */
    ...
    if (tab_name === "all-streams") {
        ...
        allStreamPage = 1;
        render_all_stream_list(true);
    } else if (tab_name === "subscribed") {
        subscribed_only = true;
        myStreamPage = 1;
        render_my_stream_list(true);
    }

    // redraw_left_panel();
    work_stream_edit.setup_subscriptions_tab_hash(tab_name);
}
...
export function show_loading() {
    const $spinner = $('#work_subscription_overlay').find(".sub_unsub_status").expectOne();
    $spinner.show();
...
export async function render_my_stream_list(isReload) {
    show_loading();
    // For annoying legacy reasons we render all the subs we are
    // allowed to know about and put them in the DOM, then we do
    // a second pass where we filter/sort them.
    const { code, result: { list , pages, total} } = await channel.get({
        url: `https://rpa.insfair.cn/zmtapi/zmt/fav/list?page=${myStreamPage}&size=${pageSize}&zulipUid=${page_params.user_id}`,
    });
    ...
    if(code === 200) {
        ...
        const html = render_work_streams_list({
            streamList: list.length === 0 ? emptyList : list,
            isAll: list.length === 0
        });

        myStreamsTotal = total;

        if(isReload) {
            scroll_util.get_content_element($("#work_streams_overlay_container .streams-list")).html(html);
        } else {
            scroll_util.get_content_element($("#work_streams_overlay_container .streams-list")).append(html);
        }
...
export async function render_all_stream_list(isReload) {
    show_loading();
    // For annoying legacy reasons we render all the subs we are
    // allowed to know about and put them in the DOM, then we do
    // a second pass where we filter/sort them.
...
export function setup_page(callback) {
    ...
    function initialize_components() {
        ...
        // 过滤按钮
        // $("#work_streams_overlay_container .search-container").prepend(sort_toggler.get());
        ...
        $("#work_streams_overlay_container .search-container").prepend(toggler_elem);
    ...
    function populate_and_fill() {
        $("#work_streams_overlay_container").empty();
        ...
        const template_data = {
            ...
            org_level_message_retention_setting:
                work_stream_edit.get_display_text_for_realm_message_retention_setting(),
        ...
        const rendered = render_work_stream_overlay(template_data);
        $("#work_streams_overlay_container").append(rendered);

        // render_left_panel_superset();
        render_my_stream_list();
        initialize_components();
        // redraw_left_panel();
        // stream_create.set_up_handlers();


        scroll_util.on_scroll_to_bottom($("#work_streams_overlay_container .streams-list"), () => {
            if (subscribed_only) {
                if (myStreamPage <= calculateTotalPages(myStreamsTotal, pageSize)) {
                    myStreamPage ++;
                    render_my_stream_list();
                }
            } else {
                if (allStreamPage <= calculateTotalPages(allStreamsTotal, pageSize)) {
                    allStreamPage ++;
                    render_all_stream_list();
                }
            }
        })

        const throttled_redraw_left_panel = _.throttle(redraw_left_panel, 50);
        $("#work_stream_filter input[type='text']").on("input", () => {
        ...
        $("#work_stream_filter input[type='text']").on("keypress", (e) => {
        ...
        $("#clear_search_stream_name").on("click", () => {
            $("#work_stream_filter input[type='text']").val("");
            redraw_left_panel();
        });
...
async function show_right_section(id) {
    ...
    if (code === 200) {
        const html = render_work_streams_detail({
            detail: result,
        });

        $("#work_streams_overlay_container .work-stream-detail").html(html);
        work_stream_edit.setup_subscriptions_stream_hash(result);
    }
...
export function launch(section) {
    setup_page(() => {
        overlays.open_overlay({
            name: "work_subscriptions",
            $overlay: $("#work_subscription_overlay"),
            on_close() {
                myStreamPage = 1;
    ...
    if (!get_active_data().id) {
        if (section === "new") {
            ...
            $("#work_search_stream_name").trigger("focus");
        }
    }
}
...
export function switch_rows(event) {
    ...
    if (hash_util.is_create_new_stream_narrow()) {
        ...
        if ($("#work_search_stream_name").is(":focus")) {
            $("#work_search_stream_name").trigger("blur");
        }
    } else {
        if (event === "up_arrow") {
        ...
        if ($("#work_search_stream_name").is(":focus")) {
            ...
            return $("#work_search_stream_name").trigger("blur");
        }
    }
...
export function open_create_stream() {
    do_open_create_stream();
    browser_history.update("#work-streams/new");
}
...
export function unsubscribe_from_private_stream(sub) {
    ...
    function unsubscribe_from_stream() {
        ...
        if (overlays.streams_open()) {
            $stream_row = $(
                "#work_streams_overlay_container div.stream-row[data-stream-id='" + sub.stream_id + "']",
            );
        }
...
async function ajaxCollectStream(id) {
    const { code, message, result } = await channel.post({
        url: `https://rpa.insfair.cn/zmtapi/zmt/fav/save?pid=${id}&zulipUid=${page_params.user_id}`,
    });

    confirm_dialog.launch({
        html_heading: '提示',
        html_body: code ===200 ? result : message,
...
async function ajaxCancelCollectStream(id) {
    const { code, result } = await channel.post({
        url: `https://rpa.insfair.cn/zmtapi/zmt/fav/cancel?pid=${id}&zulipUid=${page_params.user_id}`,
    });
...
export function initialize() {
    $("#work_streams_overlay_container").on("click", ".create_stream_button", (e) => {
        e.preventDefault();
        // open_create_stream();
        console.log('-------');
    });

    $("#work_streams_overlay_container").on("click", "#work_stream_creation_form [data-dismiss]", (e) => {
        e.preventDefault();
    ...
    $("#work_streams_overlay_container").on("click", ".email-address", function () {
        selectText(this);
    });

    // $("#work_streams_overlay_container").on(
    //     "click",
    //     ".stream-row, .create_stream_button",
    //     show_right_section,
    // );

    $("#work_streams_overlay_container").on("click", ".fa-chevron-left", () => {
        $(".right").removeClass("show");
        $(".subscriptions-header").removeClass("slide-left");
    });

    $("#work_streams_overlay_container").on("click", ".show-detail", (e) => {
        const id = $(e.currentTarget).data('id');
        show_right_section(id);
    });

    $("#work_streams_overlay_container").on("click", ".collect-stream", (e) => {
        const id = $(e.currentTarget).data('id');

        confirm_dialog.launch({
            html_heading: '提示',
            html_body: '是否确定收藏此工作流？',
            on_click: () => {
                ajaxCollectStream(id);
            },
        });
    });

    $("#work_streams_overlay_container").on("click", ".cancel-collect", (e) => {
        const id = $(e.currentTarget).data('id');

        confirm_dialog.launch({
            html_heading: '提示',
            html_body: '是否确定取消收藏此工作流？',
            on_click: async () => {
                const isSuccess = await ajaxCancelCollectStream(id);
                if(isSuccess) {
                    const $currentItem = $(e.currentTarget).closest('.work-stream-item');
                    $currentItem.fadeOut(300, function() {
                        $(this).remove();
                        if ($('.work-stream-item').length === 0) {
                            render_my_stream_list(true);
                        }
                    });
                }
