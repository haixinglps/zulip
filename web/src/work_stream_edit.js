...
export function setup_subscriptions_stream_hash(sub) {
    const hash = hash_util.work_stream_detail_url(sub);
...
export function setup_subscriptions_tab_hash(tab_key_value) {
    ...
    if (tab_key_value === "all-streams") {
        browser_history.update("#work-streams/all");
    } else if (tab_key_value === "subscribed") {
        browser_history.update("#work-streams/subscribed");
    } else {
