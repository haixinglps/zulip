...
export function setup_my_sku_hash(sub) {
    const hash = hash_util.work_sku_detail_url(sub);
...
export function setup_subscriptions_tab_hash(tab_key_value) {
    ...
    if (tab_key_value === "all-streams") {
        browser_history.update("#my-sku/all");
    } else if (tab_key_value === "subscribed") {
        browser_history.update("#my-sku/subscribed");
    } else {
